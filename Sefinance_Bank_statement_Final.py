...
"""
Reading the data from the csv files and assigning column names, separator, encoding and defining that index is not present in the csv file
and dropping columns that are completely empty
"""
data_3 = (
        pd.read_csv("SEF aug 1_20 2023 (1).csv",
                    header=None,
                    names=cols_names,
                    sep=';',
                    encoding='latin-1',
...
data_2 = (
        pd.read_csv("SEF 21_27 aug 2023 (1).csv",
                    header=None,
                    names=cols_names,
                    sep=';',
                    encoding='latin-1',
...
data_Credit=data[(data.Payment_group == 20) & (data.Payment_Type == "K")]
    """
    Adding new columns that are describing transaction types and source of the data that matches dataframe names
...
data_Credit = data_Credit[data_Credit['C'].str.contains("valid", na=False)]

    """
    At the start defining length of the first part and second part of identity code,
    then extracting this identity code from Client Info column, using regular expression, in data_Credit and data_Credit_cent dataframes, 
    then removing blank spaces from this string and naming this column without blank spaces in strings as identity_code
    at the end dropping extracted_identity_code from data_Credit and data_Credit_cent dataframes
    """
...
data_Credit_cent.drop(['extracted_identity_code'], axis=1, inplace=True)
    ...
    """
    At the start defining length of Bank Account NR,
    then creating function that will help to extract this bank account number from Client Info column, using regular expression,
    and then applying this function to each row in the dataframe and creating new column Bank_accountNR in data_Credit and data_Debet dataframes
...
data_Debet['Bank_accountNR'] = data_Debet.apply(extracting_bank_account_nr, axis=1)
    ...
    """
    Defining function that will extract agreement NR from payment_details column and defining that it is a seven digit string
    then creating new column Test_agreementNR in data_Credit dataframe using this function that was created to extract this agreement NR from payment_details column
    after that creating column agreementNR_str that is Test_agreementNR column in string format, 
    then creating replacements variable that defines all characters that need to be replaced in this string and finally creating agreementNR column 
    that is equal to agreementNR_str column, without in replacements variable defined characters and at the end dropping these two unnecessary columns 
...
df_merged_full['amount'] = df_merged_full['monthly_payment'].astype(float)
df_merged_full['amount'] = df_merged_full['monthly_payment'].astype(float)
...
def validation(row):
    ...
    elif row['amount'] == row['contract fee']:
